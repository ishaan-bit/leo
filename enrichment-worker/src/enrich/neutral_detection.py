...
TAU_EMOTION = 0.06  # Min density for explicit emotion (reduced from 0.12 - 50% reduction per user request)
TAU_EVENT = 0.06    # Min density for specific event (normalized by token count - 50% reduction)
...
def detect_neutral_states(
    ...
    # Check for explicit emotion keywords (CRITICAL: prevents false neutrality)
    STRONG_EMOTION_KEYWORDS = [
        'hate', 'angry', 'frustrated', 'furious', 'annoyed', 'irritated', 'pissed', 'mad',
        'sad', 'depressed', 'lonely', 'grief', 'heartbroken', 'devastated', 'miserable',
        'anxious', 'worried', 'scared', 'afraid', 'nervous', 'overwhelmed', 'terrified',
        'excited', 'thrilled', 'delighted', 'overjoyed',
        'disgusted', 'contempt', 'bitter', 'resentful', 'hostile',
        'demeaning', 'narcissistic', 'humiliated', 'betrayed', 'hurt'
    ]
    has_strong_emotion = any(keyword in text_lower for keyword in STRONG_EMOTION_KEYWORDS)
    
    # Compute raw evidence
    emotion_evidence = compute_emotion_evidence(text, features, emotion_valence)
    ...
    # CRITICAL FIX: Do NOT mark as neutral if strong emotion keywords detected
    if has_strong_emotion:
        is_emotion_neutral = False  # Never neutral with explicit emotion words
    else:
    ...
    if has_strong_emotion:
        parts.append("strong_emotion_keywords_detected")
